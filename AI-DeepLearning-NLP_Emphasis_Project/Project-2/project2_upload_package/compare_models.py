...
def dataframe_to_markdown(df: pd.DataFrame) -> str:
    try:
        ...
        # Fallback keeps the artifact generation robust even if tabulate is unavailable.
...
def build_metric_row(model_name: str, payload: Dict, split_name: str) -> Dict:
    ...
    # We extract only the metrics that are explicitly discussed in the paper and appendix tables.
    return {
        "model": model_name,
        "split": split_name,
...
def flatten_comparison_rows(baseline: Dict, joint: Dict) -> List[Dict]:
    ...
    for model_name, payload in [
        ...
        for split_name in ["test", "ood"]:
            # We compare only evaluation splits, not train/val, to keep the table concise.
        ...
        rows.append(
            {
                ...
                # Baseline uses the stock token-classification head without CRF/SCL extras.
        ...
        rows.append(
            {
                ...
                "epochs": cfg.get("epochs"),
                "batch_size": cfg.get("batch_size"),
                # Joint model adds both structured decoding and sentence-level representation learning.
...
def save_metric_table(rows: List[Dict], output_dir: Path) -> pd.DataFrame:
    ...
    for column in metric_columns:
        if column in pretty_df:
            # Markdown table is rounded for readability; raw CSV keeps full precision.
...
def save_comparison_plot(df: pd.DataFrame, output_dir: Path):
    ...
    # The paper highlights overall F1 together with the minority emphasis behavior.
...
def save_method_note(output_dir: Path):
    # This note documents which parts came from libraries and which parts were project-specific.
...
def main():
    ...
    summary = {
        ...
        # Summary exists mainly so later scripts or notebooks can quickly inspect generated outputs.
        "artifacts": [
