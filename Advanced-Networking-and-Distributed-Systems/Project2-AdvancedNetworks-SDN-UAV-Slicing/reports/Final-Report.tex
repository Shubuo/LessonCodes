...
\begin{enumerate}
    ...
    \item \textbf{Scenario B: SDN-Based Priority Slicing:} To protect the critical TCP traffic, we emulate an OpenFlow slicing policy using Linux Traffic Control (`tc`) and Hierarchical Token Bucket (HTB) queuing disciplines at the egress interface of $s_1$. 
...
Our quantitative results show that under heavy congestion, the Best-Effort policy is inadequate for mission-critical operations, suffering from high packet loss, high latency, and erratic jitter. The slicing policy isolates the high-priority queue, eliminating packet loss, keeping RTT stable near the propagation limit, and yielding low jitter along with improved throughput.
...
\bibitem{b11} B. Yoruk, ``Distributed MDS Simulation Framework,'' Ege University, Dec. 2025.
\end{thebibliography}
